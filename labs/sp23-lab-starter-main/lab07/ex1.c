...
long long int sum_simd(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for(unsigned int i = 0; i < NUM_ELEMS / 4 * 4; i += 4) {
            ...
            __m128i satisfied_tmp = _mm_and_si128(tmp, cmp_tmp);
            sum_vec = _mm_add_epi32(sum_vec, satisfied_tmp);
        }

        if(w % 256 == 0 || w == OUTER_ITERATIONS - 1) {
            int tmp_arr[4];
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for(unsigned int i = 0; i < NUM_ELEMS / 16 * 16; i += 16) {
            ...
            __m128i cmp_tmp1 = _mm_cmpgt_epi32(tmp1, _127);
            tmp1 = _mm_and_si128(tmp1, cmp_tmp1);
            sum_vec1 = _mm_add_epi32(sum_vec1, tmp1);

            __m128i tmp2 = _mm_loadu_si128((__m128i *)(vals + i + 4));
            __m128i cmp_tmp2 = _mm_cmpgt_epi32(tmp2, _127);
            tmp2 = _mm_and_si128(tmp2, cmp_tmp2);
            sum_vec2 = _mm_add_epi32(sum_vec2, tmp2);

            __m128i tmp3 = _mm_loadu_si128((__m128i *)(vals + i + 8));
            __m128i cmp_tmp3 = _mm_cmpgt_epi32(tmp3, _127);
            tmp3 = _mm_and_si128(tmp3, cmp_tmp3);
            sum_vec3 = _mm_add_epi32(sum_vec3, tmp3);

            __m128i tmp4 = _mm_loadu_si128((__m128i *)(vals + i + 12));
            __m128i cmp_tmp4 = _mm_cmpgt_epi32(tmp4, _127);
            tmp4 = _mm_and_si128(tmp4, cmp_tmp4);
            sum_vec4 = _mm_add_epi32(sum_vec4, tmp4);
        }

        int tmp_arr[4];
        _mm_storeu_si128((__m128i *) tmp_arr, sum_vec1);
        result += tmp_arr[0] + tmp_arr[1] + tmp_arr[2] + tmp_arr[3];
        _mm_storeu_si128((__m128i *) tmp_arr, sum_vec2);
        result += tmp_arr[0] + tmp_arr[1] + tmp_arr[2] + tmp_arr[3];
        _mm_storeu_si128((__m128i *) tmp_arr, sum_vec3);
        result += tmp_arr[0] + tmp_arr[1] + tmp_arr[2] + tmp_arr[3];
        _mm_storeu_si128((__m128i *) tmp_arr, sum_vec4);
        result += tmp_arr[0] + tmp_arr[1] + tmp_arr[2] + tmp_arr[3];
